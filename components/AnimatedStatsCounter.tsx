...
export default function AnimatedStatsCounter({ reportsCount }: { reportsCount: number }) {
  ...
  const traced = useCountUp(reportsCount, 1800, visible);
  const flagged = useCountUp(4728, 1800, visible);
  const recovered = useCountUp(41, 1800, visible);
  ...
  return (
    <div ref={ref} className="flex flex-wrap gap-6 justify-center md:justify-start">
      ...
      <div className="glass rounded-2xl px-5 py-4 text-center min-w-[110px]">
        <div className="flex items-center justify-center gap-1.5 mb-1">
          <DollarSign className="w-4 h-4" style={{ color: '#FFA500' }} />
        </div>
        <p className="font-mono font-black text-2xl" style={{ color: '#00D9FF' }}>${(recovered / 10).toFixed(1)}M+</p>
        <p className="text-xs mt-0.5" style={{ color: 'var(--text-muted)' }}>funds recovered</p>
      </div>
    </div>
  );
}
